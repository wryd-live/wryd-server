...
Router.get("/view",requireAuth,(req,res)=>{
    ...
    mysqlConnection.query("SELECT user.id,user.name,user.organization,user.verified,user.imageurl,user.email,organization.name AS orgname FROM user INNER JOIN organization ON user.organization=organization.id AND user.id=?",[userid],(err,rows,fields)=>{
        if(!err)
        {
            res.send(rows);
        }
